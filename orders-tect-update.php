...
if(isset($_SESSION['cart_tech'])) {

  $total = 0;

  foreach($_SESSION['cart_tech'] as $product_id => $quantity) {

    $result = $mysqli->query("SELECT * FROM tech_products WHERE id = ".$product_id);
    ...
    if($result){
      ...
      if($obj = $result->fetch_object()) {
        ...
        if($query){
          ...
          if($mysqli->query("UPDATE tech_products SET qty = ".$newqty." WHERE id = ".$product_id)){
